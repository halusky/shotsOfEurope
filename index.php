<!DOCTYPE html>
<html>
<head lang="en">
    <meta charset="UTF-8">
    <title>ShotsOfEurope</title>
    <link href ="stylesheet.css" type="text/css" rel="stylesheet" media="all">
    <script src='http://ajax.googleapis.com/ajax/libs/jquery/2.0.3/jquery.min.js'></script>
    <script type='text/javascript' src='shotsofeurope.js'></script>
    <script type='text/javascript' src='libraries/echo.js'></script>


</head>
<body>
    <div id = "landing">
        <div id = "background">
            <img src = "pictures/background.png">
        </div>
        <div id = "headerBox">
            <div id = "logoBox">
                <img src = "pictures/SE3white.png">

            </div>

            <div id = "navigationBox">
                <div class = "location" id = "linkLondon">  <a href = "" onclick="return false;"> London  </a> </div>
                <div class = "location" id = "linkOxford"> <a href = "" onclick="return false;"> Oxford </a> </div>
                <div class = "location" id = "linkStuttgart"> <a href = "#Stuttgart" onclick="return false;"> Stuttgart </a> </div>
                <div class = "location" id = "linkPrague"> <a href = "#Prague" onclick="return false;"> Prague </a> </div>
                <div class = "location" id = "linkOlomoucs"> <a href = "#Olomoucs" onclick="return false;"> Olomoucs </a> </div>
                <div class = "location" id = "linkZdiar"> <a href = "#Zdiar" onclick="return false;"> Zdiar </a> </div>
                <div class = "location" id = "linkBratislava"> <a href = "#Bratislava" onclick="return false;"> Bratislava </a> </div>
                <div class = "location" id = "linkVienna"> <a href = "#Vienna" onclick="return false;"> Vienna </a> </div>
                <div class = "location" id = "linkBudapest"> <a href = "#Budapest" onclick="return false;"> Budapest </a> </div>
                <div class = "location" id = "linkPecs"> <a href = "#Pecs" onclick="return false;"> Pecs </a> </div>
                <div class = "location" id = "linkParis"> <a href = "#Paris" onclick="return false;"> Paris </a> </div>
                <div class = "location" id = "linkHeidelberg"> <a href = "#Heidelberg" onclick="return false;"> Heidelberg </a> </div>
                <div class = "location" id = "linkFrankfurt"> <a href = "#Frankfurt" onclick="return false;"> Frankfurt </a> </div>
                <div class = "location" id = "linkIstanbul"> <a href = "#Istanbul" onclick="return false;"> Istanbul </a> </div>
                <div class = "location" id = "linkThessaloniki"> <a href = "#Thessaloniki" onclick="return false;"> Thessaloniki </a> </div>
                <div class = "location" id = "linkAthens"> <a href = "#Athens" onclick="return false;"> Athens </a> </div>
                <div class = "location" id = "linkRome"> <a href = "#Rome" onclick="return false;"> Rome </a> </div>
                <div class = "location" id = "linkCopenhagen"> <a href = "#Copenhagen" onclick="return false;"> Copenhagen </a> </div>

            </div>

        </div>

    </div>

    <div id = "transition">

        <img id = "transitionPic" src = "pictures/SoEgreetings.png">

    </div>


    <div id = "main">
        <div id = "navigationBox2">
            <div id = "logoBox2">
                <img src = "pictures/SE3black.png">
                <!--<img src = "pictures/SEblack.png">-->
            </div>
            <div id = "box2Items">

                <div class = "location" id = "linkLondon">  <a href = "" onclick="return false;"> London  </a> </div>
                <div class = "location" id = "linkOxford"> <a href = "" onclick="return false;"> Oxford </a> </div>
                <div class = "location" id = "linkStuttgart"> <a href = "#Stuttgart" onclick="return false;"> Stuttgart </a> </div>
                <div class = "location" id = "linkPrague"> <a href = "#Prague" onclick="return false;"> Prague </a> </div>
                <div class = "location" id = "linkOlomoucs"> <a href = "#Olomoucs" onclick="return false;"> Olomoucs </a> </div>
                <div class = "location" id = "linkZdiar"> <a href = "#Zdiar" onclick="return false;"> Zdiar </a> </div>
                <div class = "location" id = "linkBratislava"> <a href = "#Bratislava" onclick="return false;"> Bratislava </a> </div>
                <div class = "location" id = "linkVienna"> <a href = "#Vienna" onclick="return false;"> Vienna </a> </div>
                <div class = "location" id = "linkBudapest"> <a href = "#Budapest" onclick="return false;"> Budapest </a> </div>
                <div class = "location" id = "linkPecs"> <a href = "#Pecs" onclick="return false;"> Pecs </a> </div>
                <div class = "location" id = "linkParis"> <a href = "#Paris" onclick="return false;"> Paris </a> </div>
                <div class = "location" id = "linkHeidelberg"> <a href = "#Heidelberg" onclick="return false;"> Heidelberg </a> </div>
                <div class = "location" id = "linkFrankfurt"> <a href = "#Frankfurt" onclick="return false;"> Frankfurt </a> </div>
                <div class = "location" id = "linkIstanbul"> <a href = "#Istanbul" onclick="return false;"> Istanbul </a> </div>
                <div class = "location" id = "linkThessaloniki"> <a href = "#Thessaloniki" onclick="return false;"> Thessaloniki </a> </div>
                <div class = "location" id = "linkAthens"> <a href = "#Athens" onclick="return false;"> Athens </a> </div>
                <div class = "location" id = "linkRome"> <a href = "#Rome" onclick="return false;"> Rome </a> </div>
                <div class = "location" id = "linkCopenhagen"> <a href = "#Copenhagen" onclick="return false;"> Copenhagen </a> </div>
            </div>

        </div>

        <div id="container">

        <h2> London </h2>

        <div id = "picturesContainer">
            <div id = "London">
                <h1> London </h1>
                <div id = "imgLeft">
                    <img class = "Img" src = "pictures/locationPics/London - 2.jpg" alt = "">
                    <img class = "Img" src = "pictures/locationPics/London - 1.jpg" alt = "">
                    <img class = "Img" src = "pictures/locationPics/London - 3.jpg" alt = "">
                    <img class = "Img" src = "pictures/locationPics/London - 4.jpg" alt = "">
                    <img class = "Img" src = "pictures/locationPics/London - 5.jpg" alt = "">
                    <img class = "Img" src = "pictures/locationPics/London - 6.jpg" alt = "">
                </div>
                <div id = "imgRight">
                    <img class = "Img" src = "pictures/locationPics/London - 7.jpg" alt = "">
                    <img class = "Img" src = "pictures/locationPics/London - 9.jpg" alt = "">
                    <img class = "Img" src = "pictures/locationPics/London - 8.jpg" alt = "">
                    <img class = "Img" src = "pictures/locationPics/London - 20.jpg" alt = "">
                    <img class = "Img" src = "pictures/locationPics/London - 10.jpg" alt = "">

                </div>
                <div id = "imgLeft" class = "additionalPics">
                    <img class = "Img" src = "pictures/locationPics/London - 11.jpg" alt = "">
                    <img class = "Img" src = "pictures/locationPics/London - 12.jpg" alt = "">
                    <img class = "Img" src = "pictures/locationPics/London - 13.jpg" alt = "">
                    <img class = "Img" src = "pictures/locationPics/London - 14.jpg" alt = "">
                    <img class = "Img" src = "pictures/locationPics/London - 15.jpg" alt = "">
                    <img class = "Img" src = "pictures/locationPics/London - 16.jpg" alt = "">
                    <img class = "Img" src = "pictures/locationPics/London - 17.jpg" alt = "">
                    <img class = "Img" src = "pictures/locationPics/London - 18.jpg" alt = "">
                    <img class = "Img" src = "pictures/locationPics/London - 19.jpg" alt = "">

                </div>
                <div id = "imgRight" class = "additionalPics">
                    <img class = "Img" src = "pictures/locationPics/London - 21.jpg" alt = "">
                    <img class = "Img" src = "pictures/locationPics/London - 22.jpg" alt = "">
                    <img class = "Img" src = "pictures/locationPics/London - 23.jpg" alt = "">
                    <img class = "Img" src = "pictures/locationPics/London - 24.jpg" alt = "">
                    <img class = "Img" src = "pictures/locationPics/London - 25.jpg" alt = "">
                    <img class = "Img" src = "pictures/locationPics/London - 26.jpg" alt = "">
                    <img class = "Img" src = "pictures/locationPics/London - 27.jpg" alt = "">
                    <img class = "Img" src = "pictures/locationPics/London - 28.jpg" alt = "">
                    <img class = "Img" src = "pictures/locationPics/London - 29.jpg" alt = "">

                </div>
                <!--<img class = "nextImg" src = "pictures/Westminster.jpg" alt = "">-->
                <div class = "moreButton" id = "londonButton">
                    <button type = "button" onclick = ""> +more... </button>
                </div>
            </div>
        </div>

            <h2> Oxford </h2>

            <div id = "picturesContainer">
                <div id = "Oxford">
                    <h1> Oxford </h1>
                    <div id = "imgLeft">
                        <!--<img class = "Img" src="pictures/preloader.gif" alt="" data-echo="pictures/locationPics/Oxford - 1.jpg">-->
                        <img class = "Img" src = "pictures/locationPics/Oxford - 1.jpg">
                        <img class = "Img" src = "pictures/locationPics/Oxford - 2.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Oxford - 5.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Oxford - 6.jpg" alt = "">
                    </div>
                    <div id = "imgRight">
                        <img class = "Img" src = "pictures/locationPics/Oxford - 3.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Oxford - 4.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Oxford - 7.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Oxford - 8.jpg" alt = "">
                    </div>

                    <!--<div class = "moreButton" id = "oxfordButton">-->
                        <!--<button type = "button" onclick = ""> +more... </button>-->
                    <!--</div>-->

                </div>
            </div>


            <h2> Stuttgart </h2>

            <div id = "picturesContainer">

                <div id = "Stuttgart">
                    <h1> Stuttgart </h1>
                    <div id = "imgLeft">
                        <img class = "Img" src = "pictures/locationPics/Stuttgart - 1.jpg" alt = "">
                    </div>
                    <div id = "imgRight">
                        <img class = "Img" src = "pictures/locationPics/Stuttgart - 2.jpg" alt = "">
                    </div>
                </div>
            </div>

            <h2> Prague </h2>

            <div id = "picturesContainer">

                <div id = "Prague">
                    <h1> Prague </h1>
                    <div id = "imgLeft">
                        <img class = "Img" src = "pictures/locationPics/Prague - 1.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Prague - 2.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Prague - 5.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Prague - 6.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Prague - 9.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Prague - 10.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Prague - 11.jpg" alt = "">
                    </div>
                    <div id = "imgRight">
                        <img class = "Img" src = "pictures/locationPics/Prague - 3.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Prague - 4.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Prague - 7.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Prague - 8.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Prague - 12.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Prague - 13.jpg" alt = "">
                    </div>

                    <!--<div class = "moreButton" id = "pragueButton">-->
                        <!--<button type = "button" onclick = ""> +more... </button>-->
                    <!--</div>-->
                </div>
            </div>


            <h2> Olomoucs </h2>

            <div id = "picturesContainer">

                <div id = "Olomoucs">
                    <h1> Olomoucs </h1>
                    <div id = "imgLeft">
                        <img class = "Img" src = "pictures/locationPics/Olomoucs - 1.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Olomoucs - 2.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Olomoucs - 5.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Olomoucs - 6.jpg" alt = "">
                    </div>
                    <div id = "imgRight">
                        <img class = "Img" src = "pictures/locationPics/Olomoucs - 3.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Olomoucs - 4.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Olomoucs - 7.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Olomoucs - 8.jpg" alt = "">
                    </div>

                    <!--<div class = "moreButton" id = "oxfordButton">-->
                        <!--<button type = "button" onclick = ""> +more... </button>-->
                    <!--</div>-->
                </div>
            </div>


            <h2> Zdiar </h2>

            <div id = "picturesContainer">

                <div id = "Zdiar">
                    <h1> Zdiar </h1>
                    <div id = "imgLeft">
                        <img class = "Img" src = "pictures/locationPics/Zdiar - 1.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Zdiar - 2.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Zdiar - 5.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Zdiar - 7.jpg" alt = "">

                    </div>
                    <div id = "imgRight">
                        <img class = "Img" src = "pictures/locationPics/Zdiar - 3.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Zdiar - 4.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Zdiar - 6.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Zdiar - 8.jpg" alt = "">

                    </div>

                    <div id = "imgLeft" class = "additionalPics">
                        <img class = "Img" src = "pictures/locationPics/Zdiar - 9.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Zdiar - 10.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Zdiar - 11.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Zdiar - 12.jpg" alt = "">

                    </div>
                    <div id = "imgRight" class = "additionalPics">
                        <img class = "Img" src = "pictures/locationPics/Zdiar - 13.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Zdiar - 14.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Zdiar - 15.jpg" alt = "">


                    </div>

                    <div class = "moreButton" id = "oxfordButton">
                        <button type = "button" onclick = ""> +more... </button>
                    </div>
                </div>
            </div>


            <h2> Bratislava </h2>

            <div id = "picturesContainer">

                <div id = "Bratislava">
                    <h1> Bratislava </h1>
                    <div id = "imgLeft">
                        <img class = "Img" src = "pictures/locationPics/Bratislava - 1.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Bratislava - 2.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Bratislava - 5.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Bratislava - 6.jpg" alt = "">
                    </div>
                    <div id = "imgRight">
                        <img class = "Img" src = "pictures/locationPics/Bratislava - 3.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Bratislava - 4.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Bratislava - 7.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Bratislava - 8.jpg" alt = "">
                    </div>
                    <!--<div class = "moreButton" id = "oxfordButton">-->
                        <!--<button type = "button" onclick = ""> +more... </button>-->
                    <!--</div>-->
                </div>
            </div>

            <h2> Vienna </h2>

            <div id = "picturesContainer">

                <div id = "Vienna">
                    <h1> Vienna </h1>
                    <div id = "imgLeft">
                        <img class = "Img" src = "pictures/locationPics/Vienna - 1.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Vienna - 2.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Vienna - 5.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Vienna - 6.jpg" alt = "">

                    </div>
                    <div id = "imgRight">
                        <img class = "Img" src = "pictures/locationPics/Vienna - 3.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Vienna - 4.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Vienna - 7.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Vienna - 8.jpg" alt = "">

                    </div>

                    <div id = "imgLeft" class = "additionalPics">
                        <img class = "Img" src = "pictures/locationPics/Vienna - 9.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Vienna - 10.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Vienna - 11.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Vienna - 12.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Vienna - 16.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Vienna - 17.jpg" alt = "">

                    </div>
                    <div id = "imgRight" class = "additionalPics">
                        <img class = "Img" src = "pictures/locationPics/Vienna - 13.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Vienna - 14.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Vienna - 15.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Vienna - 18.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Vienna - 19.jpg" alt = "">

                    </div>

                    <!--<img class = "nextImg" src = "pictures/Westminster.jpg" alt = "">-->
                    <div class = "moreButton" id = "budapestButton">
                        <button type = "button" onclick = ""> +more... </button>
                    </div>
                </div>
            </div>



            <h2> Budapest </h2>

            <div id = "picturesContainer">

                <div id = "Budapest">
                    <h1> Budapest </h1>
                    <div id = "imgLeft">
                        <img class = "Img" src = "pictures/locationPics/Budapest - 1.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Budapest - 2.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Budapest - 5.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Budapest - 6.jpg" alt = "">

                    </div>
                    <div id = "imgRight">
                        <img class = "Img" src = "pictures/locationPics/Budapest - 3.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Budapest - 4.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Budapest - 7.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Budapest - 8.jpg" alt = "">

                    </div>

                    <div id = "imgLeft" class = "additionalPics">
                        <img class = "Img" src = "pictures/locationPics/Budapest - 9.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Budapest - 10.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Budapest - 11.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Budapest - 12.jpg" alt = "">

                    </div>
                    <div id = "imgRight" class = "additionalPics">
                        <img class = "Img" src = "pictures/locationPics/Budapest - 13.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Budapest - 14.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Budapest - 15.jpg" alt = "">

                    </div>

                    <!--<img class = "nextImg" src = "pictures/Westminster.jpg" alt = "">-->
                    <div class = "moreButton" id = "budapestButton">
                        <button type = "button" onclick = ""> +more... </button>
                    </div>
                </div>
            </div>


            <h2> Pecs </h2>

            <div id = "picturesContainer">

                <div id = "Pecs">
                    <h1> Pecs </h1>
                    <div id = "imgLeft">
                        <img class = "Img" src = "pictures/locationPics/Pecs - 1.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Pecs - 2.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Pecs - 5.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Pecs - 6.jpg" alt = "">
                    </div>
                    <div id = "imgRight">
                        <img class = "Img" src = "pictures/locationPics/Pecs - 3.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Pecs - 4.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Pecs - 7.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Pecs - 8.jpg" alt = "">
                    </div>
                    <div id = "imgLeft" class = "additionalPics">
                        <img class = "Img" src = "pictures/locationPics/Pecs - 9.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Pecs - 10.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Pecs - 11.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Pecs - 12.jpg" alt = "">

                    </div>
                    <div id = "imgRight" class = "additionalPics">
                        <img class = "Img" src = "pictures/locationPics/Pecs - 13.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Pecs - 14.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Pecs - 15.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Pecs - 16.jpg" alt = "">

                    </div>
                    <!--<img class = "nextImg" src = "pictures/Westminster.jpg" alt = "">-->
                    <div class = "moreButton" id = "pecsButton">
                        <button type = "button" onclick = ""> +more... </button>
                    </div>
                </div>
            </div>


            <h2> Paris </h2>

            <div id = "picturesContainer">

                <div id = "Paris">
                    <h1> Paris </h1>
                    <div id = "imgLeft">
                        <img class = "Img" src = "pictures/locationPics/Paris - 1.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Paris - 2.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Paris - 5.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Paris - 6.jpg" alt = "">
                    </div>
                    <div id = "imgRight">
                        <img class = "Img" src = "pictures/locationPics/Paris - 3.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Paris - 4.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Paris - 7.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Paris - 8.jpg" alt = "">
                    </div>
                    <div id = "imgLeft" class = "additionalPics">
                        <img class = "Img" src = "pictures/locationPics/Paris - 9.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Paris - 10.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Paris - 11.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Paris - 12.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Paris - 13.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Paris - 14.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Paris - 15.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Paris - 16.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Paris - 17.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Paris - 18.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Paris - 19.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Paris - 20.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Paris - 21.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Paris - 22.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Paris - 31.jpg" alt = "">

                    </div>
                    <div id = "imgRight" class = "additionalPics">
                        <img class = "Img" src = "pictures/locationPics/Paris - 23.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Paris - 24.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Paris - 25.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Paris - 26.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Paris - 27.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Paris - 28.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Paris - 29.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Paris - 30.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Paris - 36.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Paris - 32.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Paris - 33.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Paris - 34.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Paris - 35.jpg" alt = "">


                    </div>
                    <div class = "moreButton" id = "parisButton">
                        <button type = "button" onclick = ""> +more... </button>
                    </div>
                </div>
            </div>


            <h2> Heidelberg </h2>

            <div id = "picturesContainer">

                <div id = "Heidelberg">
                    <h1> Heidelberg </h1>
                    <div id = "imgLeft">
                        <img class = "Img" src = "pictures/locationPics/Heidelberg - 1.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Heidelberg - 2.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Heidelberg - 5.jpg" alt = "">

                    </div>
                    <div id = "imgRight">
                        <img class = "Img" src = "pictures/locationPics/Heidelberg - 3.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Heidelberg - 4.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Heidelberg - 6.jpg" alt = "">
                    </div>

                    <!--<div class = "moreButton" id = "oxfordButton">-->
                        <!--<button type = "button" onclick = ""> +more... </button>-->
                    <!--</div>-->
                </div>
            </div>


            <h2> Frankfurt </h2>

            <div id = "picturesContainer">

                <div id = "Frankfurt">
                    <h1> Frankfurt </h1>
                    <div id = "imgLeft">
                        <img class = "Img" src = "pictures/locationPics/Frankfurt - 1.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Frankfurt - 2.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Frankfurt - 5.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Frankfurt - 6.jpg" alt = "">
                    </div>
                    <div id = "imgRight">
                        <img class = "Img" src = "pictures/locationPics/Frankfurt - 3.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Frankfurt - 4.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Frankfurt - 7.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Frankfurt - 8.jpg" alt = "">
                    </div>

                    <!--<div class = "moreButton" id = "oxfordButton">-->
                        <!--<button type = "button" onclick = ""> +more... </button>-->
                    <!--</div>-->
                </div>
            </div>


            <h2> Istanbul </h2>

            <div id = "picturesContainer">

                <div id = "Istanbul">
                    <h1> Istanbul </h1>
                    <div id = "imgLeft">
                        <img class = "Img" src = "pictures/locationPics/Istanbul - 1.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Istanbul - 2.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Istanbul - 5.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Istanbul - 6.jpg" alt = "">
                    </div>
                    <div id = "imgRight">
                        <img class = "Img" src = "pictures/locationPics/Istanbul - 3.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Istanbul - 4.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Istanbul - 7.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Istanbul - 8.jpg" alt = "">
                    </div>
                    <div id = "imgLeft" class = "additionalPics">
                        <img class = "Img" src = "pictures/locationPics/Istanbul - 9.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Istanbul - 10.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Istanbul - 11.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Istanbul - 12.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Istanbul - 17.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Istanbul - 18.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Istanbul - 19.jpg" alt = "">
                    </div>
                    <div id = "imgRight" class = "additionalPics">
                        <img class = "Img" src = "pictures/locationPics/Istanbul - 13.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Istanbul - 14.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Istanbul - 15.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Istanbul - 16.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Istanbul - 20.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Istanbul - 21.jpg" alt = "">

                    </div>
                    <div class = "moreButton" id = "istanbulButton">
                        <button type = "button" onclick = ""> +more... </button>
                    </div>

                </div>
            </div>


            <h2> Sofia </h2>

            <div id = "picturesContainer">

                <div id = "Sofia">
                    <h1> Sofia </h1>
                    <div id = "imgLeft">
                        <img class = "Img" src = "pictures/locationPics/Sofia - 1.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Sofia - 2.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Sofia - 3.jpg" alt = "">
                    </div>
                    <div id = "imgRight">
                        <img class = "Img" src = "pictures/locationPics/Sofia - 4.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Sofia - 5.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Sofia - 6.jpg" alt = "">
                    </div>

                    <!--<div class = "moreButton" id = "oxfordButton">-->
                        <!--<button type = "button" onclick = ""> +more... </button>-->
                    <!--</div>-->
                </div>
            </div>


            <h2> Thessaloniki </h2>

            <div id = "picturesContainer">

                <div id = "Thessaloniki">
                    <h1> Thessaloniki </h1>
                    <div id = "imgLeft">
                        <img class = "Img" src = "pictures/locationPics/Thessaloniki - 1.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Thessaloniki - 2.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Thessaloniki - 5.jpg" alt = "">
                    </div>
                    <div id = "imgRight">
                        <img class = "Img" src = "pictures/locationPics/Thessaloniki - 3.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Thessaloniki - 4.jpg" alt = "">

                    </div>
                    <!--<div class = "moreButton" id = "oxfordButton">-->
                        <!--<button type = "button" onclick = ""> +more... </button>-->
                    <!--</div>-->
                </div>
            </div>


            <h2> Athens </h2>

            <div id = "picturesContainer">

                <div id = "Athens">
                    <h1> Athens </h1>
                    <div id = "imgLeft">
                        <img class = "Img" src = "pictures/locationPics/Athens - 6.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Athens - 7.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Athens - 8.jpg" alt = "">
                    </div>
                    <div id = "imgRight">
                        <img class = "Img" src = "pictures/locationPics/Athens - 9.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Athens - 10.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Athens - 11.jpg" alt = "">
                    </div>
                    <div id = "imgLeft" class = "additionalPics">


                    </div>
                    <div id = "imgRight" class = "additionalPics">


                    </div>
                    <!--<div class = "moreButton" id = "oxfordButton">-->
                        <!--<button type = "button" onclick = ""> +more... </button>-->
                    <!--</div>-->
                </div>
            </div>


            <h2> Rome </h2>

            <div id = "picturesContainer">

                <div id = "Rome">
                    <h1> Rome </h1>
                    <div id = "imgLeft">
                        <img class = "Img" src = "pictures/locationPics/Rome - 1.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Rome - 2.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Rome - 5.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Rome - 6.jpg" alt = "">
                    </div>
                    <div id = "imgRight">
                        <img class = "Img" src = "pictures/locationPics/Rome - 3.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Rome - 4.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Rome - 7.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Rome - 8.jpg" alt = "">
                    </div>
                    <div id = "imgLeft" class = "additionalPics">


                    </div>
                    <div id = "imgRight" class = "additionalPics">


                    </div>
                    <!--<div class = "moreButton" id = "romeButton" >-->
                        <!--<button type = "button" onclick = ""> +more... </button>-->
                    <!--</div>-->
                </div>
            </div>


            <h2> Copenhagen </h2>

            <div id = "picturesContainer">

                <div id = "Copenhagen">
                    <h1> Copenhagen </h1>
                    <div id = "imgLeft">
                        <img class = "Img" src = "pictures/locationPics/Copenhagen - 1.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Copenhagen - 2.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Copenhagen - 5.jpg" alt = "">
                    </div>
                    <div id = "imgRight">
                        <img class = "Img" src = "pictures/locationPics/Copenhagen - 3.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Copenhagen - 4.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Copenhagen - 7.jpg" alt = "">

                    </div>
                    <div id = "imgLeft" class = "additionalPics">
                        <img class = "Img" src = "pictures/locationPics/Copenhagen - 6.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Copenhagen - 9.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Copenhagen - 10.jpg" alt = "">

                    </div>
                    <div id = "imgRight" class = "additionalPics">
                        <img class = "Img" src = "pictures/locationPics/Copenhagen - 8.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Copenhagen - 12.jpg" alt = "">
                        <img class = "Img" src = "pictures/locationPics/Copenhagen - 11.jpg" alt = "">

                    </div>
                    <div class = "moreButton" id = "copenhagenButton">
                        <button type = "button" onclick = ""> +more... </button>
                    </div>
                </div>
            </div>


        </div>
    </div>





</body>
</html>